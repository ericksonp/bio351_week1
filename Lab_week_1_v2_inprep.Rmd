...
You are reading an R notebook. There is text that has instructions and information. Then you will see "code chunks", which are demarcated by lines that look like this:
...
What happens if you perform a mathematical function on a character variable?
```{r}
log10(professor)
```
Sometimes the error message will clearly tell you what's wrong; other times some Googling is required.
...
R can do a lot on its own, but people have written thousands of "libraries" that add additional functions for different purposes. We will use some of functions from the `data.table` and `ggplot2` and  libraries so first let's install them. `data.table` allows us to manipulate data organized in rows and columns.  `ggplot2` is the gold-standard for making beautiful graphs in R.

```{r}
install.packages(c("data.table", "ggplot2"))
```

You only need to install the packages once. So, once they are installed on your computer, you can DELETE the code above (highlight it all and delete) so that you don't re-install them if you re-run this worksheet. But, each time you load a new R session, you'll have to load them with the `library()` function to tell the computer you'll be using functions from those packages. Let's load them now.
...
```{r}
g <- fread("/Users/perickso/Documents/BIO351/H938_chr15.geno", header=TRUE)
```
...
We can see how many rows and columns are in our data.table using the dim() function
...
```{r}
g.melt<-melt(g, id.vars="p1", measure.vars=c("p11", "p12", "p22"), value.name="frequency", variable.name="genotype")
g.melt
```

**Question:** Look at the `g.melt` object. What did we just do here? The data are the same, but how has the organization changed?
...
**Question:** Do the shapes of the relationships look more linear or parabolic? Can you think of a biological explanation for why?
...
Let's add to the plot lines that represent Hardy-Weinberg proportions on top of our data. Don't worry about the code here, just know that you can use R to plot lines too!

```{r}
ggplot(g.melt)+
  geom_point(aes(x=p1,y=value,color=variable))+ #this is the same plot as before
...
**Question** How do the data compare to the Hardy-Weinberg expectations? Are any genotypes over-represented or under-represented?
...
Given the  data are from 52 sub-populations from around the globe, and alleles have some some probability of clustering within populations, a good working hypothesis for the deficiency of heterozygotes in this dataset is the presence of some population structure. *In other words, there may be some form of non-random mating such that A1A1 individuals are more likely to reproduce together, and A2A2 individuals are also more likely to reproduce together, resulting in fewer A1A2 individuals than expected by chance. *
